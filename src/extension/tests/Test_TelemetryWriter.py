...
class TestTelemetryWriter(unittest.TestCase):
    ...
    def test_write_event(self):
        ...
        self.telemetry_writer.write_event("testing telemetry write to file", Constants.EventLevel.Error, "Test Task")
        ...
        self.telemetry_writer.write_event("testing telemetry write to file", Constants.EventLevel.Error, "Test Task2")
    ...
    def test_write_event_msg_size_limit(self):
        ...
        message = "a"*3074
        self.telemetry_writer.write_event(message, Constants.EventLevel.Error, "Test Task")
        with open(os.path.join(self.telemetry_writer.events_folder_path, os.listdir(self.telemetry_writer.events_folder_path)[0]), 'r+') as f:
            events = json.load(f)
            self.assertTrue(events is not None)
            self.assertEqual(events[0]["TaskName"], "Test Task")
            self.assertTrue(len(events[0]["Message"]) < len(message.encode('utf-8')))
            chars_dropped = len(message.encode('utf-8')) - Constants.TelemetryConfig.MSG_SIZE_LIMIT_IN_CHARS + Constants.TelemetryConfig.BUFFER_FOR_DROPPED_COUNT_MSG_IN_CHARS
    ...
    def test_write_event_size_limit(self):
        ...
        message = "a"*3074
        task_name = "b"*5000
        self.telemetry_writer.write_event(task_name, message, Constants.EventLevel.Error)
    ...
    def test_delete_older_events(self):
        ...
        self.telemetry_writer.write_event("testing telemetry write to file", Constants.EventLevel.Error, "Test Task")
        self.telemetry_writer.write_event("testing telemetry write to file", Constants.EventLevel.Error, "Test Task2")
        self.telemetry_writer.write_event("testing telemetry write to file", Constants.EventLevel.Error, "Test Task3")
        ...
        self.telemetry_writer.write_event("testing telemetry write to file", Constants.EventLevel.Error, "Test Task4")
        ...
        self.assertRaises(Exception, lambda: self.telemetry_writer.write_event("testing telemetry write to file", Constants.EventLevel.Error, "Test Task4"))
