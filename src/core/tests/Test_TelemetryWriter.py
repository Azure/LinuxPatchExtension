...
class TestTelemetryWriter(unittest.TestCase):
    ...
    def mock_get_file_size(self, file_path):
        return Constants.TelemetryConfig.EVENT_FILE_SIZE_LIMIT_IN_CHARS + 10

    def mock_os_listdir(self, file_path):
        return ['testevent1.json', 'testevent2.json', 'testevent3.json', 'testevent4.json']

    def test_write_event(self):
        self.runtime.telemetry_writer.write_event("testing telemetry write to file", Constants.EventLevel.Error, "Test Task")
        ...
        self.runtime.telemetry_writer.write_event("testing telemetry write to file", Constants.EventLevel.Error, "Test Task2")
        latest_event_file = [pos_json for pos_json in os.listdir(self.runtime.telemetry_writer.events_folder_path) if re.search('^[0-9]+.json$', pos_json)][-1]
    ...
    def test_write_event_msg_size_limit(self):
        ...
        message = "a"*3074
        self.runtime.telemetry_writer.write_event(message, Constants.EventLevel.Error, "Test Task")
        latest_event_file = [pos_json for pos_json in os.listdir(self.runtime.telemetry_writer.events_folder_path) if re.search('^[0-9]+.json$', pos_json)][-1]
        with open(os.path.join(self.runtime.telemetry_writer.events_folder_path, latest_event_file), 'r+') as f:
            events = json.load(f)
            self.assertTrue(events is not None)
            ...
            chars_dropped = len(message.encode('utf-8')) - Constants.TelemetryConfig.MSG_SIZE_LIMIT_IN_CHARS + Constants.TelemetryConfig.BUFFER_FOR_DROPPED_COUNT_MSG_IN_CHARS + Constants.TelemetryConfig.EVENT_COUNTER_MSG_SIZE_LIMIT_IN_CHARS
    ...
    #     message = "a" * 3074
    #     task_name = "b" * 5000
    #     self.runtime.telemetry_writer.write_event(message, Constants.EventLevel.Error, task_name)
    ...
    #     self.runtime.telemetry_writer.write_event("testing telemetry write to file", Constants.EventLevel.Error, "Test Task")
    #     self.runtime.telemetry_writer.write_event("testing telemetry write to file", Constants.EventLevel.Error, "Test Task2")
    #     self.runtime.telemetry_writer.write_event("testing telemetry write to file", Constants.EventLevel.Error, "Test Task3")
    ...
    #     telemetry_dir_size_backup = Constants.TelemetryConfig.DIR_SIZE_LIMIT_IN_CHARS
    #     Constants.TelemetryConfig.DIR_SIZE_LIMIT_IN_CHARS = 1030
    #     telemetry_event_size_backup = Constants.TelemetryConfig.EVENT_FILE_SIZE_LIMIT_IN_CHARS
    #     Constants.TelemetryConfig.EVENT_FILE_SIZE_LIMIT_IN_CHARS = 1024
    #
    #     self.runtime.telemetry_writer.write_event("testing telemetry write to file", Constants.EventLevel.Error, "Test Task4")
    ...
    #     Constants.TelemetryConfig.DIR_SIZE_LIMIT_IN_CHARS = telemetry_dir_size_backup
    #     Constants.TelemetryConfig.EVENT_FILE_SIZE_LIMIT_IN_CHARS = telemetry_event_size_backup
    #
    ...
    #     old_event_files = [pos_json for pos_json in os.listdir(self.runtime.telemetry_writer.events_folder_path) if re.search('^[0-9]+.json$', pos_json)]
    #     telemetry_dir_size_backup = Constants.TelemetryConfig.DIR_SIZE_LIMIT_IN_CHARS
    #     Constants.TelemetryConfig.DIR_SIZE_LIMIT_IN_CHARS = 500
    #     telemetry_event_size_backup = Constants.TelemetryConfig.EVENT_FILE_SIZE_LIMIT_IN_CHARS
    #     Constants.TelemetryConfig.EVENT_FILE_SIZE_LIMIT_IN_CHARS = 400
    #     os_remove_backup = os.remove
    #     os.remove = self.mock_os_remove
    #
    #     self.assertRaises(Exception, lambda: self.runtime.telemetry_writer.write_event("testing telemetry write to file", Constants.EventLevel.Error, "Test Task4"))
    #
    #     Constants.TelemetryConfig.DIR_SIZE_LIMIT_IN_CHARS = telemetry_dir_size_backup
    #     Constants.TelemetryConfig.EVENT_FILE_SIZE_LIMIT_IN_CHARS = telemetry_event_size_backup
    ...
    def test_write_event_max_event_count_throttle_reached(self):
        event_count_max_throttle_backup = Constants.TelemetryConfig.MAX_EVENT_COUNT_THROTTLE
        Constants.TelemetryConfig.MAX_EVENT_COUNT_THROTTLE = 5
        ...
        self.runtime.telemetry_writer.write_event("testing telemetry write to file", Constants.EventLevel.Error, "Test Task5")
        ...
        Constants.TelemetryConfig.MAX_TIME_IN_SECONDS_FOR_EVENT_COUNT_THROTTLE = 0
        self.runtime.telemetry_writer.write_event("testing telemetry write to file", Constants.EventLevel.Error, "Test Task6")
        self.assertTrue(self.runtime.telemetry_writer.event_count == 2)
        Constants.TelemetryConfig.MAX_TIME_IN_SECONDS_FOR_EVENT_COUNT_THROTTLE = max_time_for_event_count_throttle_backup

        Constants.TelemetryConfig.MAX_EVENT_COUNT_THROTTLE = event_count_max_throttle_backup
    ...
    def test_write_event_with_buffer_true_and_then_flush(self):
        self.runtime.telemetry_writer.write_event_with_buffer("Message 1", Constants.EventLevel.Verbose,
                                                              Constants.BufferMessage.TRUE)
        self.runtime.telemetry_writer.write_event_with_buffer("Message 2", Constants.EventLevel.Verbose,
                                                              Constants.BufferMessage.TRUE)
        self.runtime.telemetry_writer.write_event_with_buffer("Message 3", Constants.EventLevel.Verbose,
    ...
    def test_write_event_with_buffer_true_and_then_flush_but_different_telemetry_event_level(self):
        ...
        self.runtime.telemetry_writer.write_event_with_buffer("Message 2", Constants.EventLevel.Info,
                                                              Constants.BufferMessage.FLUSH)

        # As the messages are with different EventLevel, they will be written separately
    ...
    def test_write_event_with_buffer_true_and_empty_string_and_then_flush_with_non_empty_string(self):
        self.runtime.telemetry_writer.write_event_with_buffer("", Constants.EventLevel.Verbose,
                                                              Constants.BufferMessage.TRUE)
