...
class TestUbuntuProClient(unittest.TestCase):
    ...
    def test_pro_client_attached_for_livepatching_returns_true(self):
        package_manager = self.container.get('package_manager')
        package_manager.ubuntu_pro_client.is_ubuntu_pro_client_attached = True
        self.assertTrue(package_manager.ubuntu_pro_client.pro_client_attached_for_livepatching())

    def test_pro_client_attached_for_livepatching_returns_false(self):
        package_manager = self.container.get('package_manager')
        package_manager.ubuntu_pro_client.is_ubuntu_pro_client_attached = False
        self.assertFalse(package_manager.ubuntu_pro_client.pro_client_attached_for_livepatching())

    def test_livepatch_service_enabled_on_machine_returns_true(self):
        package_manager = self.container.get('package_manager')
        self.assertTrue(package_manager.ubuntu_pro_client.livepatch_service_enabled_on_machine())

    def test_livepatch_service_enabled_on_machine_returns_false(self):
        self.runtime.set_legacy_test_type('SadPath')
        package_manager = self.container.get('package_manager')
        self.assertFalse(package_manager.ubuntu_pro_client.livepatch_service_enabled_on_machine())

    def test_livepatch_service_enabled_on_machine_exception_path(self):
        package_manager = self.container.get('package_manager')
        backup_run_command_output = package_manager.env_layer.run_command_output
        package_manager.env_layer.run_command_output = self.mock_run_command_output_raise_exception
        self.assertFalse(package_manager.ubuntu_pro_client.livepatch_service_enabled_on_machine())
    ...
    def test_livepatch_service_enabled_on_machine_returns_false_when_no_data_on_livepatch_found(self):
