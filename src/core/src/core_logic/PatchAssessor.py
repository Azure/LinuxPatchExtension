        ...
        for i in range(0, Constants.MAX_ASSESSMENT_RETRY_COUNT):
            try:
                ...
                if i <= Constants.MAX_ASSESSMENT_RETRY_COUNT:
                    ...
                    self.status_handler.add_error_to_status('Retryable error retrieving available patches: ' + repr(error), Constants.PatchOperationErrorCodes.DEFAULT_ERROR)
                    time.sleep(2*(i + 1))
                else:
                    self.composite_logger.log_error('Error retrieving available patches: ' + repr(error))
                    self.status_handler.add_error_to_status('Error retrieving available patches: ' + repr(error), Constants.PatchOperationErrorCodes.DEFAULT_ERROR)
