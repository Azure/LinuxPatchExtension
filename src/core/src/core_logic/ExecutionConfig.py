...
class ExecutionConfig(object):
    def __init__(self, env_layer, composite_logger, execution_parameters):
        ...
        self.status_complete_file_path = os.path.join(self.status_folder, str(self.sequence_number)+".complete" + ".status")
        ...
        self.composite_logger.log_debug("- Derived execution-config settings. [CoreLog={0}][StatusCompleteFile={1}][StatusFile={2}][IncludeAssessmentWithConfigurePatching={3}]".format(str(self.log_file_path), str(self.status_complete_file_path), str(self.status_file_path), self.include_assessment_with_configure_patching))
