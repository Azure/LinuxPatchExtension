...
class Dnf4PackageManager(PackageManager):
    ...
    def __init__(self, env_layer, execution_config, composite_logger, telemetry_writer, status_handler):
        super(Dnf4PackageManager, self).__init__(env_layer, execution_config, composite_logger, telemetry_writer,
                                                 status_handler)
        # Repo refresh
        self.cmd_clean_cache = "sudo dnf4 clean expire-cache"
        self.cmd_repo_refresh = "sudo dnf4 -q list updates"

        # Support to get updates and their dependencies
        self.dnf4_check = 'sudo dnf4 -q list updates'
        ...
        # Install update
        self.single_package_upgrade_cmd = 'sudo dnf4 -y install --skip-broken '

        # Package manager exit code(s)
        self.dnf4_exitcode_ok = 0
        self.dnf4_exitcode_on_no_action_for_install_update = 8
        ...
        self.dnf_utils_prerequisite = 'sudo dnf4 -y install dnf-utils'
    ...
    def invoke_package_manager_advanced(self, command, raise_on_exception=True):
        ...
        if code is self.dnf4_exitcode_ok or \
                (any(command_expecting_no_action_exitcode in command for command_expecting_no_action_exitcode in
                     self.commands_expecting_no_action_exitcode) and
                 code is self.dnf4_exitcode_on_no_action_for_install_update):
            self.composite_logger.log_debug(
                '[DNF4] Invoked package manager. [Command={0}][Code={1}][Output={2}]'.format(command, str(code),
                                                                                             str(out)))
        else:
            self.composite_logger.log_warning(
                '[ERROR] Customer environment error. [Command={0}][Code={1}][Output={2}]'.format(command, str(code),
                                                                                                 str(out)))
            error_msg = "Customer environment error: Investigate and resolve unexpected return code ({0}) from package manager on command: {1}".format(
                str(code), command)
            self.status_handler.add_error_to_status(error_msg,
                                                    Constants.PatchOperationErrorCodes.PACKAGE_MANAGER_FAILURE)
    ...
    def get_all_updates(self, cached=False):
        ...
        if cached and not len(self.all_updates_cached) == 0:
            self.composite_logger.log_debug(
                "[DNF4] Get all updates : [Cached={0}][PackagesCount={1}]]".format(str(cached),
                                                                                   len(self.all_updates_cached)))
        ...
        out = self.invoke_package_manager(self.add_additional_parameters_as_required_to_cmd(self.dnf4_check))
        self.all_updates_cached, self.all_update_versions_cached = self.extract_packages_and_versions(out)
        self.composite_logger.log_debug("[DNF4] Get all updates : [Cached={0}][PackagesCount={1}]]".format(str(False),
                                                                                                           len(self.all_updates_cached)))
    ...
    def get_security_updates(self):
        ...
        self.composite_logger.log_verbose(
            "[DNF4] Discovering all packages as 'security' packages, since DNF4 does not support package classification...")
        security_packages, security_package_versions = self.get_all_updates(cached=False)
        self.composite_logger.log_debug(
            "[DNF4] Discovered 'security' packages. [Count={0}]".format(len(security_packages)))
    ...
    def extract_packages_and_versions_including_duplicates(self, output):
        ...
        for line_index in range(0, len(lines)):
            ...
            if len(line) == 3 and self.__is_package(line[0]):
                ...
                self.composite_logger.log_verbose(
                    "[DNF4] > Inapplicable line (" + str(line_index) + "): " + lines[line_index])
    ...
    def get_all_available_versions_of_package(self, package_name):
        ...
        # Loaded plugin: dnf4repogpgcheck
    ...
    def is_package_version_installed(self, package_name, package_version):
        ...
        self.composite_logger.log_verbose(
            "[DNF4] Checking package install status. [PackageName={0}][PackageVersion={1}]".format(str(package_name),
                                                                                                   str(package_version)))
        ...
        for index, package in enumerate(packages):
            if package == package_name and (package_versions[index] == package_version):
                self.composite_logger.log_debug(
                    "[DNF4] > Installed version match found. [PackageName={0}][PackageVersion={1}]".format(
                        str(package_name), str(package_version)))
                return True
            else:
                self.composite_logger.log_verbose(
                    "[DNF4] > Did not match: " + package + " (" + package_versions[index] + ")")
        ...
        self.composite_logger.log_debug(
            "[DNF4] > Installed version match NOT found. [PackageName={0}][PackageVersion={1}]".format(
                str(package_name), str(package_version)))
        return False
    ...
    def add_arch_dependencies(self, package_manager, package, version, packages, package_versions,
                              package_and_dependencies, package_and_dependency_versions):
        """
        ...
        for possible_arch_dependency, possible_arch_dependency_version in zip(packages, package_versions):
            if package_manager.get_product_name_without_arch(
                    possible_arch_dependency) == package_name_without_arch and possible_arch_dependency not in package_and_dependencies and possible_arch_dependency_version == version:
    ...
    def is_valid_update(self, package_details_in_output, package_arch_to_look_for):
        ...
        #   python3-libs                       x86_64                    3.12.3-5.azl3                      azurelinux-official-base   36.05M                      10.52M
        return len(package_details_in_output) == 6 and self.is_arch_in_package_details(package_details_in_output[1],
                                                                                       package_arch_to_look_for)
    ...
    def get_dependent_list(self, packages):
        ...
        self.composite_logger.log_verbose(
            "[DNF4] Resolved dependencies. [Command={0}][Packages={1}][DependencyCount={2}]".format(str(cmd),
                                                                                                    str(packages),
                                                                                                    len(dependencies)))
        return dependencies
    ...
    def get_current_auto_os_patch_state(self):
        ...
        self.composite_logger.log("[DNF4] OS patch state per auto OS update service: [dnf-automatic={0}]".format(
        ...
        self.composite_logger.log_debug(
            "[DNF4] Overall Auto OS Patch State based on all auto OS update service states [OverallAutoOSPatchState={0}]".format(
    ...
    def __get_current_auto_os_patch_state_for_dnf_automatic(self):
        ...
        self.composite_logger.log_debug(
            "[DNF4] Fetching current automatic OS patch state in dnf-automatic service. This includes checks on whether the service is installed, current auto patch enable state and whether it is set to enable on reboot")
    ...
    def __get_current_auto_os_updates_setting_on_machine(self):
        ...
        try:
            ...
            if download_updates_value == "":
                self.composite_logger.log_debug("[DNF4] Machine did not have any value set for [Setting={0}]".format(
                    str(self.download_updates_identifier_text)))
            else:
                self.composite_logger.log_verbose(
                    "[DNF4] Current value set for [{0}={1}]".format(str(self.download_updates_identifier_text),
                                                                    str(download_updates_value)))

            if apply_updates_value == "":
                self.composite_logger.log_debug("[DNF4] Machine did not have any value set for [Setting={0}]".format(
                    str(self.apply_updates_identifier_text)))
            else:
                self.composite_logger.log_verbose(
                    "[DNF4] Current value set for [{0}={1}]".format(str(self.apply_updates_identifier_text),
            ...
            raise Exception(
                "[DNF4] Error occurred in fetching current auto OS update settings from the machine. [Exception={0}]".format(
                    repr(error)))
    ...
    def is_auto_update_service_installed(self, install_check_cmd):
        ...
        self.composite_logger.log_debug(
            "[DNF4] Checked if auto update service is installed. [Command={0}][Code={1}][Output={2}]".format(
                install_check_cmd, str(code), out))
        if len(out.strip()) > 0 and code == 0:
            self.composite_logger.log_debug("[DNF4] > Auto OS update service is installed on the machine")
            return True
        else:
            self.composite_logger.log_debug("[DNF4] > Auto OS update service is NOT installed on the machine")
            return False
    ...
    def is_service_set_to_enable_on_reboot(self, command):
        ...
        code, out = self.env_layer.run_command_output(command, False, False)
        self.composite_logger.log_debug(
            "[DNF4] Checked if auto update service is set to enable on reboot. [Code={0}][Out={1}]".format(str(code),
                                                                                                           out))
        if len(out.strip()) > 0 and code == 0 and 'enabled' in out:
            self.composite_logger.log_debug("[DNF4] > Auto OS update service will enable on reboot")
            return True
        self.composite_logger.log_debug("[DNF4] > Auto OS update service will NOT enable on reboot")
        return False
    ...
    def disable_auto_update_on_reboot(self, command):
        ...
        if code != 0:
            self.composite_logger.log_error(
                "[DNF4][ERROR] Error disabling auto update on reboot. [Command={0}][Code={1}][Output={2}]".format(
                    command, str(code), out))
            ...
            self.composite_logger.log_debug(
                "[DNF4] Disabled auto update on reboot. [Command={0}][Code={1}][Output={2}]".format(command, str(code),
                                                                                                    out))
    ...
    def backup_image_default_patch_configuration_if_not_exists(self):
        ...
        try:
            self.composite_logger.log_debug(
                "[DNF4] Ensuring there is a backup of the default patch state for [AutoOSUpdateService={0}]".format(
                    str(self.current_auto_os_update_service)))
            ...
            is_backup_valid = self.is_image_default_patch_configuration_backup_valid(
                image_default_patch_configuration_backup)
            if is_backup_valid:
                self.composite_logger.log_debug(
                    "[DNF4] Since extension has a valid backup, no need to log the current settings again. [Default Auto OS update settings={0}] [File path={1}]"
                    .format(str(image_default_patch_configuration_backup),
                            self.image_default_patch_configuration_backup_path))
            else:
                self.composite_logger.log_debug(
                    "[DNF4] Since the backup is invalid, will add a new backup with the current auto OS update settings")
                self.composite_logger.log_debug(
                    "[DNF4] Fetching current auto OS update settings for [AutoOSUpdateService={0}]".format(
                        str(self.current_auto_os_update_service)))
                is_service_installed, enable_on_reboot_value, download_updates_value, apply_updates_value = self.__get_current_auto_os_updates_setting_on_machine()
                ...
                self.composite_logger.log_debug(
                    "[DNF4] Logging default system configuration settings for auto OS updates. [Settings={0}] [Log file path={1}]"
                    .format(str(image_default_patch_configuration_backup),
                            self.image_default_patch_configuration_backup_path))
                self.env_layer.file_system.write_with_retry(self.image_default_patch_configuration_backup_path,
                                                            '{0}'.format(
                                                                json.dumps(image_default_patch_configuration_backup)),
                                                            mode='w+')
        except Exception as error:
            error_message = "[DNF4] Exception during fetching and logging default auto update settings on the machine. [Exception={0}]".format(
                repr(error))
    ...
    def is_backup_valid_for_dnf_automatic(self, image_default_patch_configuration_backup):
        if self.dnf_auto_os_update_service in image_default_patch_configuration_backup \
                and self.dnf_automatic_download_updates_identifier_text in image_default_patch_configuration_backup[
            self.dnf_auto_os_update_service] \
                and self.dnf_automatic_apply_updates_identifier_text in image_default_patch_configuration_backup[
            self.dnf_auto_os_update_service] \
                and self.dnf_automatic_enable_on_reboot_identifier_text in image_default_patch_configuration_backup[
            self.dnf_auto_os_update_service] \
                and self.dnf_automatic_installation_state_identifier_text in image_default_patch_configuration_backup[
            self.dnf_auto_os_update_service]:
            self.composite_logger.log_debug(
                "[DNF4] Extension has a valid backup for default dnf-automatic configuration settings")
            return True
        else:
            self.composite_logger.log_debug(
                "[DNF4] Extension does not have a valid backup for default dnf-automatic configuration settings")
            return False
    ...
    def update_os_patch_configuration_sub_setting(self, patch_configuration_sub_setting, value="no",
        ...
        try:
            ...
            self.composite_logger.log_debug(
                "[DNF4] Updating system configuration settings for auto OS updates. [Patch Configuration Sub Setting={0}] [Value={1}]".format(
            ...
            error_msg = "[DNF4] Error occurred while updating system configuration settings for auto OS updates. [Patch Configuration={0}] [Error={1}]".format(
    ...
    def revert_auto_os_update_to_system_default(self):
        ...
        self.composite_logger.log(
            "[DNF4] Reverting the current automatic OS patch state on the machine to its system default value before patchmode was set to 'AutomaticByPlatform'")
    ...
    def revert_auto_os_update_to_system_default_for_dnf_automatic(self):
        ...
        self.__init_auto_update_for_dnf_automatic()
        self.composite_logger.log(
            "[DNF4] Reverting the current automatic OS patch state on the machine to its system default value for [Service={0}]".format(
                str(self.current_auto_os_update_service)))
        is_service_installed, enable_on_reboot_value, download_updates_value, apply_updates_value = self.__get_current_auto_os_updates_setting_on_machine()

        if not is_service_installed:
            self.composite_logger.log_debug(
                "[DNF4] Machine default auto OS update service is not installed on the VM and hence no config to revert. [Service={0}]".format(
                    str(self.current_auto_os_update_service)))
            return

        self.composite_logger.log_debug(
            "[DNF4] Logging current configuration settings for auto OS updates [Service={0}][Is_Service_Installed={1}][Machine_default_update_enable_on_reboot={2}][{3}={4}]][{5}={6}]"
            .format(str(self.current_auto_os_update_service), str(is_service_installed), str(enable_on_reboot_value),
                    str(self.download_updates_identifier_text), str(download_updates_value),
                    str(self.apply_updates_identifier_text), str(apply_updates_value)))

        image_default_patch_configuration_backup = self.__get_image_default_patch_configuration_backup()
        self.composite_logger.log_debug(
            "[DNF4] Logging system default configuration settings for auto OS updates. [Settings={0}]".format(
                str(image_default_patch_configuration_backup)))
        is_backup_valid = self.is_image_default_patch_configuration_backup_valid(
            image_default_patch_configuration_backup)

        if is_backup_valid:
            download_updates_value_from_backup = \
            image_default_patch_configuration_backup[self.current_auto_os_update_service][
                self.download_updates_identifier_text]
            ...
            self.update_os_patch_configuration_sub_setting(self.download_updates_identifier_text,
                                                           download_updates_value_from_backup,
                                                           self.auto_update_config_pattern_match_text)
            self.update_os_patch_configuration_sub_setting(self.apply_updates_identifier_text,
                                                           apply_updates_value_from_backup,
                                                           self.auto_update_config_pattern_match_text)
            ...
            self.composite_logger.log_debug(
                "[DNF4] Since the backup is invalid or does not exist for current service, we won't be able to revert auto OS patch settings to their system default value. [Service={0}]".format(
                    str(self.current_auto_os_update_service)))
    ...
    def enable_auto_update_on_reboot(self):
        ...
        if code != 0:
            self.composite_logger.log_error(
                "[DNF4][ERROR] Error enabling auto update on reboot. [Command={0}][Code={1}][Output={2}]".format(
                    command, str(code), out))
            error_msg = 'Unexpected return code (' + str(code) + ') on command: ' + command
            self.status_handler.add_error_to_status(error_msg, Constants.PatchOperationErrorCodes.OPERATION_FAILED)
            raise Exception(error_msg, "[{0}]".format(Constants.ERROR_ADDED_TO_STATUS))
            ...
            self.composite_logger.log_debug(
                "[DNF4] Enabled auto update on reboot. [Command={0}][Code={1}][Output={2}]".format(command, str(code),
                                                                                                   out))
    ...
    def __get_image_default_patch_configuration_backup(self):
        ...
        if self.image_default_patch_configuration_backup_exists():
            try:
                ...
                self.composite_logger.log_error(
                    "[DNF4] Unable to read backup for default patch state. Will attempt to re-write. [Exception={0}]".format(
                        repr(error)))
    ...
    def is_reboot_pending(self):
        ...
        try:
            ...
            self.composite_logger.log_debug(
                "[DNF4] > Reboot required debug flags (dnf4): " + str(pending_file_exists) + ", " + str(
                    pending_processes_exist) + ".")
    ...
    def do_processes_require_restart(self):
        ...
        self.composite_logger.log_verbose("[DNF4] Checking if process requires reboot")
        # Checking using dnf-utils
        code, out = self.env_layer.run_command_output(self.dnf_utils_prerequisite, False,
                                                      False)  # idempotent, doesn't install if already present
        self.composite_logger.log_verbose(
            "[DNF4] Idempotent dnf-utils existence check. [Code={0}][Out={1}]".format(str(code), out))
        ...
        self.composite_logger.log_verbose(
            "[DNF4] > Code: " + str(code) + ", Output: \n|\t" + "\n|\t".join(out.splitlines()))
        if out.find("Reboot is required") < 0:
            self.composite_logger.log_debug("[DNF4] > Reboot not detected to be required (L1).")
        else:
            self.composite_logger.log_debug("[DNF4] > Reboot is detected to be required (L1).")
            return True
