...
class TelemetryWriter(object):
    ...
    def __init__(self, env_layer, composite_logger, events_folder_path, telemetry_supported):
        ...
        self.write_event('Started AzGPS Linux Patch Extension (Core) telemetry.', Constants.EventLevel.Info)
        self.machine_info = None
        self.__set_and_write_machine_config_info()
    ...
    def write_config_info(self, config_info, config_type='unknown'):
        ...
        return self.write_event(payload_json, Constants.EventLevel.Info)
    ...
    def write_package_info(self, package_name, package_ver, package_size, install_dur, install_result, code_path, install_cmd, output=''):
        ...
        self.write_event(message, Constants.EventLevel.Info)
    ...
    # Composed payload
    def __set_and_write_machine_config_info(self):
        ...
        self.machine_info = "[PlatformName={0}][PlatformVersion={1}][MachineCpu={2}][MachineArch={3}][DiskType={4}]".format(
                             ...
                             self.__get_machine_processor(), str(self.env_layer.platform.machine()), self.__get_disk_type())
        self.write_event("[TW] System configuration for troubleshooting. {0}".format(self.machine_info), Constants.EventLevel.Info)
    ...
    def write_execution_error(self, cmd, code, output):
        ...
        return self.write_event(error_payload, Constants.EventLevel.Error)
    # endregion

    # region Machine config retrieval methods
    def __get_machine_processor(self):
    ...
    def __new_event_json(self, event_level, message, task_name):
        return {
            ...
            "EventPid": "",
            "EventTid": "",
            "OperationId": self.__operation_id  # activity id from config settings
        }
    ...
    def __ensure_message_restriction_compliance(self, full_message):
        ...
        try:
            ...
            if len(formatted_message.encode('utf-8')) + Constants.TelemetryConfig.EVENT_COUNTER_MSG_SIZE_LIMIT_IN_CHARS > message_size_limit_in_chars:
                ...
                chars_dropped = len(formatted_message) - message_size_limit_in_chars + Constants.TelemetryConfig.BUFFER_FOR_DROPPED_COUNT_MSG_IN_CHARS + Constants.TelemetryConfig.EVENT_COUNTER_MSG_SIZE_LIMIT_IN_CHARS
                formatted_message = formatted_message[:message_size_limit_in_chars - Constants.TelemetryConfig.BUFFER_FOR_DROPPED_COUNT_MSG_IN_CHARS - Constants.TelemetryConfig.EVENT_COUNTER_MSG_SIZE_LIMIT_IN_CHARS].decode('utf-8') + '. [{0} chars dropped]'.format(chars_dropped)
    ...
    def write_event_with_buffer(self, message, event_level, buffer_msg):
        if buffer_msg == Constants.BufferMessage.TRUE and (event_level == self.last_telemetry_event_level or self.last_telemetry_event_level is None):
            if self.telemetry_buffer_store != "":
                self.telemetry_buffer_store = self.telemetry_buffer_store + self.TELEMETRY_BUFFER_DELIMITER + message
            else:
    ...
    def write_event(self, message, event_level=Constants.EventLevel.Info, task_name=Constants.TelemetryTaskName.UNKNOWN, is_event_file_throttling_needed=True):
        ...
        try:
            ...
            self.composite_logger.log_telemetry_module_error("[TW] Internal reporting error in writing telemetry events. [Error={0}]".format(repr(e)))
            # Not raising an exception here because legitimate bugs can cause this in an isolated fashion without affecting other telemetry writes in the same operation / VM.
    ...
    def __throttle_telemetry_writes_if_required(self, is_event_file_throttling_needed=True):
        ...
        try:
            ...
            if time_from_throttle_start_check_total_seconds < Constants.TelemetryConfig.MAX_TIME_IN_SECONDS_FOR_EVENT_COUNT_THROTTLE:
                ...
                if self.event_count >= Constants.TelemetryConfig.MAX_EVENT_COUNT_THROTTLE - 1:
                    end_time_for_event_count_throttle_check = self.start_time_for_event_count_throttle_check + datetime.timedelta(seconds=Constants.TelemetryConfig.MAX_TIME_IN_SECONDS_FOR_EVENT_COUNT_THROTTLE)
                    ...
                    self.write_event(message=event_write_throttled_msg, event_level=Constants.EventLevel.Info, is_event_file_throttling_needed=False)
